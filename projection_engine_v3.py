...
class CurrentRosterFilter:
    ...
    def __init__(self, roster_df: Optional[pd.DataFrame] = None):
        ...
        self._info_by_team_key: Dict[str, Dict[str, Dict[str, object]]] = {}
        self._prepare()
    ...
    def _prepare(self) -> None:
        ...
        # Official row lookup by team + normalized player name.
        # This is used to make the current roster cache authoritative for
        # position/team identity in upcoming-game depth charts. Without this,
        # matched players keep their historical stat-row position (for example,
        # a player listed as SSDM in current_rosters.csv can still display as M
        # if his latest historical game row used M).
        self._info_by_team_key: Dict[str, Dict[str, Dict[str, object]]] = {}
        for tid, g in df.groupby("engine_team_id"):
            team_map: Dict[str, Dict[str, object]] = {}
            for _, rr in g.iterrows():
                nk = str(rr.get("name_key", ""))
                if not nk:
                    continue
                official_pos = _norm_pos(str(rr.get("Position", "M")))
                team_map[nk] = {
                    "official_player_name": str(rr.get("Player", "")).strip(),
                    "official_first_name": str(rr.get("First_Name", "")).strip(),
                    "official_last_name": str(rr.get("Last_Name", "")).strip(),
                    "official_position": official_pos,
                    "official_team_id": str(rr.get("engine_team_id", tid)).strip().upper(),
                    "official_team_name": str(rr.get("Team", "")).strip(),
                    "official_team_code": str(rr.get("Team_Code", "")).strip().upper(),
                    "official_position_group": str(rr.get("Position_Group", "")).strip(),
                    "official_jersey": rr.get("Jersey", ""),
                    "official_image_slug": str(rr.get("Image_Slug", "")).strip(),
                    "official_image_url": str(rr.get("Image_URL", "")).strip(),
                    "official_roster_source_url": str(rr.get("Page_URL", "")).strip(),
                }
            self._info_by_team_key[str(tid)] = team_map
    ...
    def team_info_map(self, team_id: str) -> Dict[str, Dict[str, object]]:
        """Return {normalized_player_name: official roster metadata} for one team."""
        return self._info_by_team_key.get(str(team_id).upper(), {})
    ...
    def filter_player_rows(self, team_id: str, roster_latest: pd.DataFrame) -> Tuple[Optional[pd.DataFrame], Dict[str, object]]:
        ...
        matched = tmp[tmp["_name_key"].isin(official_keys)].copy()

        # Critical: after filtering by official roster membership, also overwrite
        # position/team display fields with the official current roster values.
        # Historical PLL stat rows can carry broad or stale positions (M vs SSDM,
        # SSDM vs M, etc.). Depth charts for current/future games should reflect
        # current_rosters.csv, not the last historical stat-row label.
        info_map = self.team_info_map(team_id)
        corrected_positions = 0
        corrected_names = 0
        for idx, rr in matched.iterrows():
            nk = str(rr.get("_name_key", ""))
            info = info_map.get(nk, {})
            if not info:
                continue

            official_pos = _norm_pos(str(info.get("official_position", "M")))
            old_pos = _norm_pos(str(rr.get("pos_norm", rr.get("position_norm", rr.get("position", "M")))))
            if official_pos != old_pos:
                corrected_positions += 1

            # Set every position field because _project_player() checks pos_norm
            # first, then position_norm, then position.
            matched.at[idx, "position"] = official_pos
            matched.at[idx, "position_norm"] = official_pos
            matched.at[idx, "pos_norm"] = official_pos
            matched.at[idx, "official_position"] = official_pos
            matched.at[idx, "official_position_group"] = info.get("official_position_group", "")

            # Also make team/display metadata current-roster authoritative.
            matched.at[idx, "team_id"] = str(team_id).upper()
            if info.get("official_team_name"):
                matched.at[idx, "team_name"] = info.get("official_team_name")
            if info.get("official_player_name"):
                corrected_names += int(str(rr.get("full_name", "")).strip() != str(info.get("official_player_name", "")).strip())
                matched.at[idx, "full_name"] = info.get("official_player_name")
            if info.get("official_first_name"):
                matched.at[idx, "first_name"] = info.get("official_first_name")
            if info.get("official_last_name"):
                matched.at[idx, "last_name"] = info.get("official_last_name")

            matched.at[idx, "official_team_code"] = info.get("official_team_code", "")
            matched.at[idx, "official_jersey"] = info.get("official_jersey", "")
            matched.at[idx, "official_image_slug"] = info.get("official_image_slug", "")
            matched.at[idx, "official_roster_source_url"] = info.get("official_roster_source_url", "")
            matched.at[idx, "official_roster_match"] = 1

        matched = matched.drop(columns=["_name_key"], errors="ignore").copy()

        detail["matched_count"] = int(len(matched))
        detail["official_position_corrections"] = int(corrected_positions)
        detail["official_name_corrections"] = int(corrected_names)
...
class RatingBuilder:
    ...
    def build_player_ratings(self) -> pd.DataFrame:
        ...
        # Merge team totals for share computation (left join on game_id + team_id).
        # Do NOT cast to str -- astype(str) converts NaN to literal "nan" which
        # breaks valid joins. Keep native types; fill missing totals after merge.
        tg_totals = self.tg[["game_id", "team_id"] + [
            c for c in ("goals", "assists", "shots") if c in self.tg.columns
        ]].rename(columns={"goals": "team_goals", "assists": "team_assists",
                           "shots": "team_shots"}).copy()
        pg = pg.merge(tg_totals, on=["game_id", "team_id"], how="left")
        # Fill any missing team totals with league averages so shares are never zero
