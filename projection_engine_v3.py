...
def _name_key(x: str) -> str:
    """Stable player-name key used to match PLL roster-page names to stat rows."""
    if x is None:
        return ""
    x = str(x).strip().lower()
...
def _slug_name_key(slug: str) -> str:
    return _name_key(str(slug or "").split("/")[-1].replace("-", " "))


# Official PLL roster pages.  These are only used to filter the projection
# roster; if a page fails or parses too few players, the model falls back to
# the existing latest-team logic so projections still run.
TEAM_ROSTER_URLS: Dict[str, List[str]] = {
    "ATL": [
        "https://premierlacrosseleague.com/teams/new-york-atlas/roster",
        "https://stats.premierlacrosseleague.com/teams/atlas",
    ],
    "CAN": [
        "https://premierlacrosseleague.com/teams/boston-cannons/roster",
        "https://stats.premierlacrosseleague.com/teams/cannons",
    ],
    "RED": [
        "https://premierlacrosseleague.com/teams/california-redwoods/roster",
        "https://premierlacrosseleague.com/teams/carolina-redwoods/roster",  # historical/redirect fallback
        "https://stats.premierlacrosseleague.com/teams/redwoods",
    ],
    "CHA": [
        "https://premierlacrosseleague.com/teams/carolina-chaos/roster",
        "https://stats.premierlacrosseleague.com/teams/chaos",
    ],
    "OUT": [
        "https://premierlacrosseleague.com/teams/denver-outlaws/roster",
        "https://stats.premierlacrosseleague.com/teams/outlaws",
    ],
    "WHP": [
        "https://premierlacrosseleague.com/teams/maryland-whipsnakes/roster",
        "https://stats.premierlacrosseleague.com/teams/whipsnakes",
    ],
    "WAT": [
        "https://premierlacrosseleague.com/teams/philadelphia-waterdogs/roster",
        "https://stats.premierlacrosseleague.com/teams/waterdogs",
    ],
    "ARC": [
        "https://premierlacrosseleague.com/teams/utah-archers/roster",
        "https://stats.premierlacrosseleague.com/teams/archers",
    ],
}

# Team-code normalization for roster sources.
# The official public PLL roster pages use market/team abbreviations such as
# BOS/NY/UTA, while the projection warehouse uses canonical stat IDs such as
# CAN/ATL/ARC. Normalize everything before matching rosters to stat rows.
ROSTER_TEAM_ID_MAP: Dict[str, str] = {
    "BOS": "CAN", "CANNONS": "CAN", "BOSTON CANNONS": "CAN", "CAN": "CAN",
    "CAL": "RED", "REDWOODS": "RED", "CALIFORNIA REDWOODS": "RED", "RED": "RED",
    "CAR": "CHA", "CHAOS": "CHA", "CAROLINA CHAOS": "CHA", "CHA": "CHA",
    "DEN": "OUT", "OUTLAWS": "OUT", "DENVER OUTLAWS": "OUT", "OUT": "OUT",
    "MD": "WHP", "MARYLAND": "WHP", "WHIPSNAKES": "WHP", "MARYLAND WHIPSNAKES": "WHP", "WHP": "WHP",
    "NY": "ATL", "NEW YORK": "ATL", "ATLAS": "ATL", "NEW YORK ATLAS": "ATL", "ATL": "ATL",
    "PHI": "WAT", "PHILADELPHIA": "WAT", "WATERDOGS": "WAT", "PHILADELPHIA WATERDOGS": "WAT", "WAT": "WAT",
    "UTA": "ARC", "UTAH": "ARC", "ARCHERS": "ARC", "UTAH ARCHERS": "ARC", "ARC": "ARC",
}

def _normalize_roster_team_id(value: str) -> str:
    raw = str(value or "").strip().upper()
    raw = re.sub(r"\s+", " ", raw)
    return ROSTER_TEAM_ID_MAP.get(raw, raw)


class ActiveRosterProvider:
    """Load current active roster names for projection depth-chart filtering.

    Load order:
      1. Cached CSV produced by pll_current_roster_cache.py
      2. Working Playwright scraper extracted from the Google Sheets updater
      3. Lightweight static HTML fallback

    The projection model still uses historical stat rows for ratings; this class
    only decides which historical player rows are eligible for current rosters.
    """

    def __init__(self, timeout: int = 8, min_players: int = 12, max_players: int = 45):
        self.timeout = timeout
        self.min_players = min_players
        self.max_players = max_players
        self.source_notes: Dict[str, str] = {}
        self.last_source: str = "unavailable"
        self.last_error: str = ""

    def load_current_rosters(self) -> Dict[str, Set[str]]:
        disabled = str(os.getenv("PLL_USE_LIVE_ROSTERS", "1")).strip().lower() in {"0", "false", "no", "off"}
        if disabled:
            self.last_source = "disabled"
            logger.info("Live PLL roster scraping disabled by PLL_USE_LIVE_ROSTERS=0")
            return {}

        csv_rosters = self._load_rosters_from_csv()
        if csv_rosters:
            self.last_source = "cached_csv"
            return csv_rosters

        # The official PLL roster cards are client-rendered. The working roster
        # updater uses Playwright/Chromium, so try that before the static parser.
        playwright_rosters = self._load_rosters_with_playwright()
        if playwright_rosters:
            self.last_source = "playwright_live"
            return playwright_rosters

        static_rosters = self._load_rosters_static()
        if static_rosters:
            self.last_source = "static_html_live"
    ...
    def _candidate_csv_paths(self) -> List[Path]:
        ...
        env_path = os.getenv("PLL_ACTIVE_ROSTER_CSV")
        if env_path:
            candidates.append(Path(env_path))
        candidates.extend([
            Path(__file__).parent / "data" / "reference_tables" / "current_rosters.csv",
            Path(__file__).parent / "data" / "current_rosters.csv",
            Path(__file__).parent / "current_rosters.csv",
        ])
        return candidates

    @staticmethod
    def _find_col(df: pd.DataFrame, names: Set[str]) -> Optional[str]:
    ...
    def _df_to_rosters(self, df: pd.DataFrame, source_label: str) -> Dict[str, Set[str]]:
        if df is None or df.empty:
            return {}

        team_col = self._find_col(df, {"team_id", "engine_team_id", "team_code", "team", "teamid"})
        name_col = self._find_col(df, {"full_name", "player", "player_name", "name"})
        if not team_col or not name_col:
            self.last_error = f"{source_label}: missing team/name columns"
            return {}

        rosters: Dict[str, Set[str]] = {}
        for _, r in df.dropna(subset=[team_col, name_col]).iterrows():
            tid = _normalize_roster_team_id(str(r[team_col]))
            key = _name_key(str(r[name_col]))
            if tid and key:
                rosters.setdefault(tid, set()).add(key)

        usable = {tid: vals for tid, vals in rosters.items() if self.min_players <= len(vals) <= self.max_players}
        if usable:
            for tid, vals in usable.items():
                self.source_notes[tid] = f"{source_label}:{len(vals)} players"
        return usable

    def _load_rosters_from_csv(self) -> Dict[str, Set[str]]:
        for fp in self._candidate_csv_paths():
            if not fp.exists():
                continue
            try:
                df = pd.read_csv(fp)
                rosters = self._df_to_rosters(df, str(fp))
                if rosters:
                    logger.info("Active rosters loaded from CSV %s: %s", fp, {k: len(v) for k, v in rosters.items()})
                    return rosters
            except Exception as exc:
                self.last_error = f"Could not load active roster CSV {fp}: {exc}"
                logger.warning(self.last_error)
        return {}

    def _load_rosters_with_playwright(self) -> Dict[str, Set[str]]:
        enabled = str(os.getenv("PLL_ENABLE_PLAYWRIGHT_ROSTERS", "1")).strip().lower() not in {"0", "false", "no", "off"}
        if not enabled:
            return {}
        try:
            from pll_current_roster_cache import scrape_all_pll_rosters_sync
            roster_df, diagnostics_df = scrape_all_pll_rosters_sync()
            rosters = self._df_to_rosters(roster_df, "playwright")
            if rosters:
                logger.info("Active rosters loaded by Playwright: %s", {k: len(v) for k, v in rosters.items()})
                self._try_write_cache(roster_df)
                return rosters
            self.last_error = "Playwright scrape returned no usable roster groups"
        except Exception as exc:
            self.last_error = f"Playwright roster scrape unavailable: {exc}"
            logger.warning(self.last_error)
        return {}

    def _try_write_cache(self, roster_df: pd.DataFrame) -> None:
        if str(os.getenv("PLL_WRITE_ROSTER_CACHE", "1")).strip().lower() in {"0", "false", "no", "off"}:
            return
        if roster_df is None or roster_df.empty:
            return
        fp = Path(__file__).parent / "data" / "reference_tables" / "current_rosters.csv"
        try:
            fp.parent.mkdir(parents=True, exist_ok=True)
            roster_df.to_csv(fp, index=False)
            logger.info("Wrote active roster cache: %s", fp)
        except Exception as exc:
            logger.debug("Could not write roster cache %s: %s", fp, exc)

    def _load_rosters_static(self) -> Dict[str, Set[str]]:
        out: Dict[str, Set[str]] = {}
        for team_id, urls in TEAM_ROSTER_URLS.items():
            names: Set[str] = set()
            used_url = None
            for url in urls:
                try:
                    html = self._fetch(url)
                    parsed = self._extract_names_from_html(html)
                    if self.min_players <= len(parsed) <= self.max_players:
                        names = parsed
                        used_url = url
                        break
                except Exception as exc:
                    logger.debug("Static roster fetch/parse failed for %s %s: %s", team_id, url, exc)
                    continue
            if names:
                out[team_id] = names
                self.source_notes[team_id] = used_url or "static_html"
                logger.info("Active roster loaded for %s: %d players", team_id, len(names))
            else:
                logger.warning("No usable active roster scraped for %s; using latest-team fallback", team_id)
        return out
    ...
    def _extract_names_from_html(self, html: str) -> Set[str]:
        names: Set[str] = set()
        if not html:
            return names
        ...
        try:
            ...
            for a in soup.find_all("a"):
                ...
                if "/player" in href and text:
                    k = _name_key(text)
                    if self._looks_like_name_key(k):
                        names.add(k)
                if "/player" in href:
                    slug = href.rstrip("/").split("/")[-1]
                    k = _slug_name_key(slug)
                    if self._looks_like_name_key(k):
                        names.add(k)
        except Exception:
            pass
        ...
        json_patterns = [
            r'"firstName"\s*:\s*"([^"{}]{1,40})".{0,240}?"lastName"\s*:\s*"([^"{}]{1,50})"',
            r'"lastName"\s*:\s*"([^"{}]{1,50})".{0,240}?"firstName"\s*:\s*"([^"{}]{1,40})"',
        ]
        for i, pat in enumerate(json_patterns):
            for m in re.finditer(pat, html, flags=re.IGNORECASE | re.DOTALL):
                if i == 0:
                    first, last = m.group(1), m.group(2)
                else:
                    last, first = m.group(1), m.group(2)
                k = _name_key(f"{first} {last}")
                if self._looks_like_name_key(k):
                    names.add(k)

        for m in re.finditer(r'"(?:slug|playerSlug|slugname)"\s*:\s*"([a-z0-9]+(?:-[a-z0-9]+){1,4})"', html, flags=re.IGNORECASE):
            k = _slug_name_key(m.group(1))
            if self._looks_like_name_key(k):
                names.add(k)
    ...
    @staticmethod
    def _looks_like_name_key(k: str) -> bool:
        if not k:
            return False
        parts = k.split()
        if not (2 <= len(parts) <= 4):
            return False
        blocked = {
            "new york", "boston cannons", "california redwoods", "carolina chaos",
            "denver outlaws", "maryland whipsnakes", "philadelphia waterdogs",
            "utah archers", "premier lacrosse", "privacy policy", "terms conditions",
        }
        if k in blocked:
            return False
        return all(len(p) >= 2 or p in {"td", "tj", "jr"} for p in parts)
...
class RatingBuilder:
    ...
    def get_team_rating(self, team_id: str, as_of_date=None) -> Dict:
        ...
        if as_of_date and "game_date_utc" in df.columns:
            ...
            mask &= df["game_date_utc"] < cutoff
        sub = df[mask]
        if sub.empty:
            logger.warning("No ratings for team_id=%s", team_id)
            return {}
        row = sub.sort_values(["season", "game_number"]).iloc[-1]
        return {k: _nan(v) if isinstance(v, float) else v for k, v in row.to_dict().items()}
    ...
    def get_player_rating(self, player_id: str, as_of_date=None) -> Dict:
        ...
        if sub.empty:
            return {}
        row = sub.sort_values(["season", "game_date_utc"]).iloc[-1]
        return {k: _nan(v) if isinstance(v, float) else v for k, v in row.to_dict().items()}
...
class PlayerModel:
    ...
    def __init__(self, player_ratings: pd.DataFrame,
                 ...
                 current_roster_names: Optional[Dict[str, Set[str]]] = None):
        ...
        self._current_rosters = current_roster_names or {}
    ...
    def project_roster(
        ...
        # Prefer official current PLL roster pages when available.  The old
        # latest-team map is still a fallback, but it can keep players who are
        # no longer on the active team after offseason cuts/transfers.
        active_name_keys = self._current_rosters.get(str(team_id), set())
        used_live_roster = False
        if active_name_keys:
            def _row_keys(r: pd.Series) -> Set[str]:
                keys: Set[str] = set()
                for c in ("full_name", "player_name", "name"):
                    if c in r and pd.notna(r[c]):
                        ...
                        if k:
                            keys.add(k)
                if "player_slug" in r and pd.notna(r["player_slug"]):
                    k = _slug_name_key(str(r["player_slug"]))
                    if k:
                        keys.add(k)
                if "first_name" in r and "last_name" in r and pd.notna(r.get("first_name")) and pd.notna(r.get("last_name")):
                    k = _name_key(f"{r.get('first_name')} {r.get('last_name')}")
                    if k:
                        keys.add(k)
                return keys

            mask = roster_latest.apply(lambda r: bool(_row_keys(r) & active_name_keys), axis=1)
            matched = int(mask.sum())
            # Keep the live roster filter only when it matches enough historical
            # stat rows to be trustworthy. Some rookies will not match because
            # they do not yet have PLL game history, so the threshold is modest.
            min_match = min(8, max(4, int(len(active_name_keys) * 0.25)))
            if matched >= min_match:
                before = len(roster_latest)
                roster_latest = roster_latest[mask].copy()
                used_live_roster = True
                logger.info(
                    "Roster filter %s: %d -> %d players using official roster names",
                    team_id, before, len(roster_latest),
                )
            else:
                logger.warning(
                    "Ignoring live roster filter for %s: only %d matched rows from %d parsed names",
                    team_id, matched, len(active_name_keys),
                )

        # Fallback: filter to players whose most recent team in game logs is this team.
        # Goalies are no longer exempt when official roster names are available.
        if (not used_live_roster) and self._current_team:
        ...
        for _, row in roster_latest.iterrows():
            ...
            usage = float(po.get("usage_multiplier", 1.0))
...
class ProjectionEngine:
    ...
    def __init__(self, db_path: Optional[str] = None, hold_pct: float = 0.045):
        ...
        self.current_rosters: Dict[str, Set[str]] = {}
        self.current_roster_source: str = "not_loaded"
        self.current_roster_notes: Dict[str, str] = {}
        self.current_roster_error: str = ""
    ...
    def fit(self, run_backtest: bool = False) -> Optional[BacktestResult]:
        ...
        roster_provider = ActiveRosterProvider()
        self.current_rosters = roster_provider.load_current_rosters()
        self.current_roster_source = roster_provider.last_source
        self.current_roster_notes = roster_provider.source_notes
        self.current_roster_error = roster_provider.last_error
        pr = self.rating_builder._pr
        self.player_model = PlayerModel(
            pr if pr is not None else pd.DataFrame(),
            current_team_map=current_team_map,
            current_roster_names=self.current_rosters,
        )
        self._fitted = True
