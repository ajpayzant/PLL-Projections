...
class DataLoader:
    ...
    def resolve_current_team(self, player_games: pd.DataFrame) -> Dict[str, str]:
        if player_games.empty:
            return {}
        latest = (
            player_games.sort_values(["season", "game_number"])
            .groupby("player_id")[["team_id"]].last()
            .reset_index()
        )
        return dict(zip(latest["player_id"].astype(str), latest["team_id"].astype(str)))
...
class PlayerModel:
    ...
    def project_roster(
        ...
        # Filter to players whose most recent team is this team.
        # Exception: always keep goalies for this team regardless of current_team_map —
        # goalies rarely transfer mid-season and the map sometimes misdirects them
        # when they appeared for another team earlier in their career.
        if self._current_team:
            goalie_pids = set(
                roster_latest[roster_latest["pos_norm"] == "G"]["player_id"].astype(str).tolist()
            )
            roster_latest = roster_latest[
                roster_latest["player_id"].astype(str).map(
                    lambda pid: (self._current_team.get(pid, team_id) == team_id)
                    or (pid in goalie_pids)   # always keep goalies for this team
                )
            ].copy()
...
class PricingEngine:
    ...
    def _opt_line(self, dist: np.ndarray) -> float:
        if len(dist) == 0:
            return 0.5
        max_l = float(np.percentile(dist, 99)) + 5.0
        cands = np.arange(0.5, max_l + 0.5, 0.5)
        best, best_d = cands[0], float("inf")
        for c in cands:
            d = abs(float(np.mean(dist > c)) - 0.50)
            if d < best_d:
                best_d, best = d, c
        return float(best)
