...
with st.sidebar:
    ...
    st.markdown('<span class="note-text">Override any line to see how odds shift at that price.</span>',
                unsafe_allow_html=True)

    use_tot = st.checkbox("Override total line", key="ov_tot_chk")
    cust_tot = st.number_input("Total line", 10.0, 40.0, float(gm.total_line), 0.5,
                               key="cust_tot", disabled=not use_tot)

    use_spd = st.checkbox("Override spread", key="ov_spd_chk")
    cust_spd = st.number_input(f"Spread ({home_nm})", -15.0, 15.0,
                               float(gm.spread_home), 0.5,
...
with c1:
    ...
    st.markdown(card(lbl, aspd_odds, f"P(cover): {1-p_hcover:.1%}"), unsafe_allow_html=True)
with c2:
    note = " ⚡" if use_spd else ""
    st.markdown(card(f"Spread{note}", f"{act_spread:+.1f}", "model spread"), unsafe_allow_html=True)
with c3:
    lbl = f"{home_nm} {act_spread:+.1f}" if act_spread != 0 else f"{home_nm} PK"
    st.markdown(card(lbl, hspd_odds, f"P(cover): {p_hcover:.1%}"), unsafe_allow_html=True)
...
summary = pd.DataFrame([
    {"Market": f"{away_nm} ML",            "Line": "—",               "Odds": gm.away_ml,  "Fair Prob": fmt_prob(gm.away_win_prob)},
    {"Market": f"{home_nm} ML",            "Line": "—",               "Odds": gm.home_ml,  "Fair Prob": fmt_prob(gm.home_win_prob)},
    {"Market": f"{away_nm} {away_spd:+.1f}", "Line": f"{away_spd:+.1f}", "Odds": aspd_odds,   "Fair Prob": fmt_prob(1-p_hcover)},
    {"Market": f"{home_nm} {act_spread:+.1f}", "Line": f"{act_spread:+.1f}", "Odds": hspd_odds, "Fair Prob": fmt_prob(p_hcover)},
    {"Market": "Total Over",               "Line": str(act_total),    "Odds": over_odds,   "Fair Prob": fmt_prob(p_over)},
    {"Market": "Total Under",              "Line": str(act_total),    "Odds": under_odds,  "Fair Prob": fmt_prob(1-p_over)},
])
...
n = len(hs)
grid = [[float(np.mean((hs == hv) & (as_ == av))) * 100 for hv in h_vals] for av in a_vals]
...
fig.update_layout(
    ...
    xaxis_title=f"{home_nm} Score", yaxis_title=f"{away_nm} Score",
...
fig2.add_vline(x=act_total, line_dash="dash", line_color="#f59e0b", line_width=2,
...
st.markdown(f'<span class="note-text">Engine v3 · {gs.n_sims:,} sims · '
            f'Hold: {hold_slider*100:.1f}%</span>', unsafe_allow_html=True)
