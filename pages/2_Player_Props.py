...
from _engine_state import (
    SHARED_CSS, card, fmt_prob, pos_badge,
...
with st.sidebar:
    ...
    show_pos   = st.multiselect("Positions", ["A","M","D","FO","SSDM","LSM","G"],
    ...
    st.markdown('<span class="note-text">Type a player name and stat to price at a custom line.</span>',
                unsafe_allow_html=True)
    ov_player = st.text_input("Player name (partial)", key="ov_player")
    ov_stat   = st.selectbox("Stat", ["goals","assists","points","shots_on_goal","saves","faceoff_wins"],
                             key="ov_stat")
    ov_line   = st.number_input("Line", 0.0, 20.0, 0.5, 0.5, key="ov_line")
...
st.markdown(f"**{len(sims_filtered)} players shown**")
st.markdown("---")
    ...
    with st.expander(
        ...
        with col_info:
            ...
            if pos == "G":
                ...
                st.markdown(f"Proj FO Wins: **{proj.proj_faceoff_wins:.2f}**")
                st.markdown(f"FO%: **{proj.proj_faceoff_pct:.3f}**")
            else:
                st.markdown(f"Goals: **{proj.proj_goals:.3f}**  Assists: **{proj.proj_assists:.3f}**")
        ...
        with col_dist:
            pri_stat = "saves" if pos == "G" else ("faceoff_wins" if pos == "FO" else "points")
            if pri_stat in ps.stat_distributions:
                dist = ps.stat_distributions[pri_stat]
                fig = go.Figure(go.Histogram(x=dist, nbinsx=20,
                                            marker_color=team_color(tid), opacity=0.75))
                pv_val = pv.get(pri_stat, 0)
                ...
                fig.update_layout(
                    ...
                    xaxis_title=STAT_LABELS.get(pri_stat, pri_stat), yaxis_title="",
                )
...
st.markdown("---")
st.markdown(f'<span class="note-text">Hold: {hold_pct*100:.1f}% · 20,000 sims · '
            'Adjust hold on the Projections page.</span>', unsafe_allow_html=True)
